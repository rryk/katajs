...
/** @define {string} */
XMOT.version = 'DEVELOPMENT SNAPSHOT (June 15 2012)';
...
XMOT.normalizeVector = normalizeVector;
    ...
    p.translate = function(translation){
		if(this.constraint.constrainTranslation(translation, this))
			...
			this.transform.translation = this.transform.translation.add( new XML3DVec3(translation[0],translation[1],translation[2]) );
		return this;
    };
...
(function(){
	...
	m.createMoveable = function(obj, constraint){
		var t = org.xml3d.URIResolver.resolve(obj.transform, obj.ownerDocument);
		if (!t) {
	...
	m.createAnimatable = function(obj, constraint){
		...
		// DIRTY HACK !!
		//
		// Somehow check whether doc is already in correct format, only turn to XML3D doc, if not
		doc = new org.xml3d.XML3DDocument(obj.ownerDocument);
		var t = org.xml3d.URIResolver.resolve(doc, obj.transform);
		if (!t) {
			throw "Object does not have a transfrom property.";
			return null;
		}
    ...
    m.createKeyframeAnimation = function(name, type, element, opt){
		...
		var keys = convertArrToInt(child.firstChild.textContent.split(' '));

		child = child.nextElementSibling;
		var values = convertArrToInt(child.firstChild.textContent.split(' '));
		if(!keys || !values){
		...
		else if(type === "orientation" || type == "Orientation")
